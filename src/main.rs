...
async fn upload(State(state): State<AppState>, mut multipart: Multipart) -> Response {
    ...
    let mut file_data: Option<(String, std::path::PathBuf)> = None;
        ...
        if name == "year" {
            if let Ok(text) = field.text().await {
                if !text.trim().is_empty() {
                    year_val = Some(text.trim().to_string());
                }
            }
        } else if name == "file" {
            ...
            file_data = Some((file_name, temp_path));
        }
    }

    if let Some((file_name, temp_path)) = file_data {
        let job_id = uuid::Uuid::new_v4().to_string();
        ...
        tokio::spawn(async move {
            if file_name_clone.ends_with(".docx") {
                ...
                for card_result in parser.parse_records() {
                    match card_result {
                        Ok(mut card) => {
                            // Use provided year if CSV didn't have one
                            if card.year.is_empty() {
                                if let Some(y) = &year_clone {
                                    card.year = y.clone();
                                }
                            }
                            batch.push(card);
                ...
                match tokio::fs::read(&temp_path).await {
                    Ok(data) => {
                        ...
                        let year_for_thread = year_clone.clone();

                        let all_cards: Vec<Card> = tokio::task::spawn_blocking(move || {
                            files_to_parse_shared.par_iter().filter_map(|(name, buf)| {
                                let parser = CardParser::new(name.clone(), buf.clone(), year_for_thread.clone());
