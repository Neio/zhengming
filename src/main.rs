...
async fn get_progress(Path(id): Path<String>, State(state): State<AppState>) -> impl IntoResponse {
...
async fn upload(State(state): State<AppState>, mut multipart: Multipart) -> Response {
    while let Ok(Some(field)) = multipart.next_field().await {
        ...
        if name == "file" {
            ...
            let mut stream = StreamReader::new(field.map_err(std::io::Error::other));

            if let Err(e) = tokio::io::copy(&mut stream, &mut file).await {
                let _ = tokio::fs::remove_file(&temp_path).await;
                return (
                    StatusCode::INTERNAL_SERVER_ERROR,
                    format!("Failed to stream upload: {}", e),
                )
                    .into_response();
            }
            ...
            state
                ...
                .write()
                .unwrap()
                .insert(job_id.clone(), initial_progress.clone());
            ...
            tokio::spawn(async move {
                if file_name_clone.ends_with(".docx") {
                    ...
                    match tokio::fs::read(&temp_path).await {
                        Ok(data) => {
                            job_progress_clone.write().unwrap().status =
                                "Parsing docx...".to_string();
                            ...
                            match parser.parse() {
                                Ok(cards) => {
                                    ...
                                    match state_clone.index.add_cards(&cards) {
                                        Ok(_) => {
                                            job_progress_clone.write().unwrap().cards_uploaded =
                                                cards.len();
                                            job_progress_clone.write().unwrap().status =
                                                "Completed".to_string();
                                        }
                                        Err(e) => {
                                            job_progress_clone.write().unwrap().error =
                                                Some(format!("Indexing error: {}", e))
                                        }
                                    }
                                }
                                Err(e) => {
                                    job_progress_clone.write().unwrap().error =
                                        Some(format!("Parser error: {}", e))
                                }
                            }
                        }
                        Err(e) => {
                            job_progress_clone.write().unwrap().error =
                                Some(format!("Failed to read temp file: {}", e))
                        }
                    }
                    ...
                    let file = match std::fs::File::open(&temp_path) {
                        ...
                        Err(e) => {
                            job_progress_clone.write().unwrap().error =
                                Some(format!("Failed to open temp file: {}", e));
                            return;
                        }
                    };

                    let parser = OpenCaselistParser::new(BufReader::new(file));
                    let mut writer = match state_clone.index.create_writer(100_000_000) {
                        // 100MB heap
                        Ok(w) => w,
                        Err(e) => {
                            job_progress_clone.write().unwrap().error =
                                Some(format!("Failed to create Tantivy writer: {}", e));
                            return;
                        }
                    };
                        ...
                        match card_result {
                            Ok(card) => {
                                ...
                                if batch.len() >= 5000 {
                                    ...
                                    if let Err(e) =
                                        state_clone.index.add_cards_to_writer(&mut writer, &batch)
                                    {
                                        job_progress_clone.write().unwrap().error =
                                            Some(format!("Batch indexing error: {}", e));
                                        return;
                                    }
                                    if let Err(e) = writer.commit() {
                                        job_progress_clone.write().unwrap().error =
                                            Some(format!("Batch commit error: {}", e));
                                        return;
                                    }
                                    batch.clear();
                            ...
                            Err(e) => {
                                job_progress_clone.write().unwrap().error =
                                    Some(format!("CSV Parser error: {}", e));
                                return;
                            }
                        }
                    }
                    ...
                    match tokio::fs::read(&temp_path).await {
                        Ok(data) => {
                            ...
                            let mut archive = match zip::ZipArchive::new(cursor) {
                                ...
                                Err(e) => {
                                    job_progress_clone.write().unwrap().error =
                                        Some(format!("Failed to read ZIP: {}", e));
                                    return;
                                }
                            };

                            job_progress_clone.write().unwrap().status =
                                "Unzipping Archive...".to_string();
                            let mut files_to_parse = Vec::new();
                            for i in 0..archive.len() {
                                if let Ok(mut file) = archive.by_index(i) {
                                    let entry_name = file.name().to_string();
                                    if entry_name.ends_with(".docx")
                                        && !entry_name.contains("__MACOSX")
                                    {
                                        let mut buf = Vec::new();
                            ...
                            let all_cards: Vec<Card> = tokio::task::spawn_blocking(move || {
                                files_to_parse_shared
                                    .par_iter()
                                    .filter_map(|(name, buf)| {
                                        let parser = CardParser::new(name.clone(), buf.clone());
                                        let parsed = parser.parse().ok();
                                        let done =
                                            processed_ref.fetch_add(1, Ordering::Relaxed) + 1;
                                        if done.is_multiple_of(10) || done == file_count {
                                            progress_for_thread.write().unwrap().processed_files =
                                                done;
                                        }
                                        parsed
                                    })
                            ...
                            if !all_cards.is_empty() {
                                ...
                                job_progress_clone.write().unwrap().error =
                                    Some("No valid .docx files found in the ZIP".to_string());
                            }
                        }
                        Err(e) => {
                            job_progress_clone.write().unwrap().error =
    ...
    (
        ...
        "No valid .docx, .csv or .zip file found in request",
    )
        .into_response()
}
...
async fn get_stats(State(state): State<AppState>) -> Response {
            ...
            if let Ok(progress) = job.read() {
                if progress.status != "Completed" && progress.error.is_none() {
                    let pending = progress
                        .cards_indexed
                        .saturating_sub(progress.cards_uploaded);
    ...
    if let Some(obj) = stats.as_object_mut() {
        obj.insert(
            "pending_cards".to_string(),
            serde_json::json!(pending_cards),
        );
    }
...
mod tests {
    ...
    fn test_zip_extraction() {
        ...
        assert!(
            docx_count > 0,
            "Zip file should contain at least one valid docx file"
        );
    }
}
